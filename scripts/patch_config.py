...
def discover_product_files(products_dir: Path) -> dict[str, list[str]]:
    ...
    for f in products_dir.glob("*"):
        ...
        if suffix_lower == ".rnx":
            ...
            if f.name != "igs_satellite_metadata.snx":  # static file lives in the template
    ...
    for key in product_files:
        product_files[key].sort()

    return product_files


def patch_config_with_products(config_path: Path, product_files: dict[str, list[str]]) -> None:
    ...
    if product_files["snx_files"]:
        existing_snx = cfg["inputs"].get("snx_files", [
    ...
    for prod_type, files in product_files.items():
        if files:
            logging.info("  %s: %s", prod_type, ", ".join(files))


def patch_config_for_date(date: str, work_root: Path, source_config: Path, config_name: str) -> bool:
...
def main():
    ...
    range_parser.add_argument("--config", required=True,
                               help="Config file to copy and patch")
    ...
    range_parser.add_argument("--work-root", type=Path, required=True,
                               help="Base work directory")

    single_parser = subparsers.add_parser("single", help="Patch a single config file in place")
    single_parser.add_argument("--config", type=Path, required=True,
                                help="Config file to patch (modified in place)")
    single_parser.add_argument("--products-dir", type=Path, required=True,
                                help="Directory containing product files")
    single_parser.add_argument("--output-dir", type=Path,
                                help="Set outputs_root in config to this directory")
    ...
    if args.command == "range":
        ...
        if not source_config.exists():
            logging.error("Config file not found: %s", source_config)
            return 1
        ...
        n_ok = sum(
            patch_config_for_date(d, args.work_root, source_config, source_config.name)
            for d in dates
        )
        ...
        try:
            patch_config_with_products(args.config, discover_product_files(args.products_dir))
